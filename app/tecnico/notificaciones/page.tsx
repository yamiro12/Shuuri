...
import { TECNICOS, NOTIFICACIONES } from '@/data/mock';
...
export default function TecnicoNotificaciones() {
  const searchParams = useSearchParams();
  const tecnicoId    = searchParams.get('id') ?? 'T001';
  const TECNICO      = TECNICOS.find(t => t.id === tecnicoId) ?? TECNICOS[0];

  const initial = useMemo(
    () => NOTIFICACIONES.filter(n => n.actorId === tecnicoId && n.actorTipo === 'TECNICO'),
    [tecnicoId]
  );
                ...
                {TODOS_TIPOS.map(t => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-semibold transition-colors capitalize border-r last:border-r-0 ${
                      filtroTipo === t ? 'bg-[#0D0D0D] text-white' : 'text-gray-500 hover:bg-gray-50'
                    }`}
                  >
              ...
              {([
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold transition-colors border-r last:border-r-0 ${
                    filtroLeida === f.key ? 'bg-[#0D0D0D] text-white' : 'text-gray-500 hover:bg-gray-50'
                  }`}
                >
                  {f.label}
                </button>
